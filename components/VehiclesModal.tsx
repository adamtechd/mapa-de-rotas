...
interface VehiclesModalProps {
  ...
  onDeleteCallback: (vehicleId: string) => void;
}

const VehiclesModal: React.FC<VehiclesModalProps> = ({ isOpen, onClose, vehicles, onSave, onDeleteCallback }) => {
  const [items, setItems] = useState<Vehicle[]>([]);
  const [newItemName, setNewItemName] = useState('');

  useEffect(() => {
    if (isOpen) {
      setItems(JSON.parse(JSON.stringify(vehicles)));
    }
  }, [isOpen, vehicles]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] flex flex-col">
        <div className="p-6 border-b">
          <h2 className="text-2xl font-bold text-slate-800">Gerenciar Veículos</h2>
        </div>
